...
const getMonthlyConsumptionMap = async (medicineIds) => {
  ...
  }).select('medicine_id sold_units').lean();

  return logs.reduce((consumption, log) => {
    consumption[log.medicine_id] = (consumption[log.medicine_id] || 0) + (log.sold_units || 0);
    return consumption;
  }, {});
};
...
export const runForecasting = async (req, res) => {
  try {
    ...
    const consumptionMap = await getMonthlyConsumptionMap(medicines.map((medicine) => medicine.medicine_id));
    ...
    if (predictions.length > 0) {
      await Medicine.bulkWrite(
        predictions.map((p) => ({
          updateOne: {
            filter: { medicine_id: p.medicine_id },
            update: {
